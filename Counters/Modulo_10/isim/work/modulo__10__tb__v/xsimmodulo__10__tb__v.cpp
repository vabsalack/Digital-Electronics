...
va_list vap)
{
    extern HSim__s6 * createworkMmodulo__10__tb__v(const char*);
    HSim__s6 *blk = createworkMmodulo__10__tb__v(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkM_modulo__10(const char*);
    HSim__s6 *blk = createworkM_modulo__10(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","modulo_10_tb_v", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","Modulo_10", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMmodulo__10__tb__v(const char*);
        topvl = (HSim__s5*)createworkMmodulo__10__tb__v("modulo_10_tb_v");
