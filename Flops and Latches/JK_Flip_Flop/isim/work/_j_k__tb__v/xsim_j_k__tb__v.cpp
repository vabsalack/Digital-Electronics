...
va_list vap)
{
    extern HSim__s6 * createworkM_j_k__tb__v(const char*);
    HSim__s6 *blk = createworkM_j_k__tb__v(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkM_j_k___f_f(const char*);
    HSim__s6 *blk = createworkM_j_k___f_f(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","JK_tb_v", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","JK_FF", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkM_j_k__tb__v(const char*);
        topvl = (HSim__s5*)createworkM_j_k__tb__v("JK_tb_v");
