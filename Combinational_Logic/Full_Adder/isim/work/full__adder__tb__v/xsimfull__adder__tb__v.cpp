...
va_list vap)
{
    extern HSim__s6 * createworkMfull__adder__tb__v(const char*);
    HSim__s6 *blk = createworkMfull__adder__tb__v(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkM_full__adder(const char*);
    HSim__s6 *blk = createworkM_full__adder(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","full_adder_tb_v", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","Full_adder", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMfull__adder__tb__v(const char*);
        topvl = (HSim__s5*)createworkMfull__adder__tb__v("full_adder_tb_v");
