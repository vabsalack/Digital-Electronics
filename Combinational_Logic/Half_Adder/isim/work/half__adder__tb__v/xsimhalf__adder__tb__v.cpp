...
va_list vap)
{
    extern HSim__s6 * createworkMhalf__adder__tb__v(const char*);
    HSim__s6 *blk = createworkMhalf__adder__tb__v(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","half_adder_tb_v", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","half_adder", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMhalf__adder__tb__v(const char*);
        topvl = (HSim__s5*)createworkMhalf__adder__tb__v("half_adder_tb_v");
